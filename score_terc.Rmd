...
foreach(b=1:nboot, .packages = 'easyVerification') %dopar%  { # loop on bootstrap samples
  ...
  #scoreprob_q_ps <- array (NA, dim=c(ngrids,ndimT,e))
  scoreprob_seas_ps <- array (NA, dim=c(ngrids,e))
    
    # Qinzaine scoring
    
  # for (t in 1:ndimT_period){
  #   scoreprob_q_ps[,t,1] <- veriApply("EnsRoca", fcst=array_prev[,,,t], obs=array_obs[,,t], prob=1:2/3, parallel = TRUE)$cat1 
  #   scoreprob_q_ps[,t,2] <- veriApply("EnsRoca", fcst=array_prev[,,,t], obs=array_obs[,,t], prob=1:2/3, parallel = TRUE)$cat3 
  # }
  ...
  #nom_prevmont <- paste0(indicateur, b,"scoreprobqz_grid_prev.RData")
  nom_prevseas <- paste0(indicateur, b,"scoreprobseason_grid_prev.RData")

  #save(scoreprob_q_ps, file = file.path(dir_data, "score", nom_prevmont))
...
for (indicateur in indic_period){
  ...
  for (b in 1:nboot){
    ...
    nom_prevseas <- paste0(indicateur,b,"_seasprob_prev.RData")
    nom_refseas <- paste0(indicateur,b,"_seasbin_obs.RData")
    load(file = file.path(dir_data, "extract", nom_prevseas))
    load(file = file.path(dir_data, "extract", nom_refseas))
    vect_event2 <- vector()
    for (e in 1:3){ #LT,MT,HT
      
      vect_event2[e] <- score_boite_prob(seasprob_ps, seasbin_ref, lat, nb_bin = 5, e)$AUC # seasonal 
  ...
  nom_seas <- paste0(indicateur,"boxprob_seas.RData")
...
indicateur <- "DRAIN"
  ...
  boxboot <- array(unlist(list_boot2) , c(event,nboot))
   ...
   wilcox.test(boxboot[2,], mu = 0.5, alternative = "greater", conf.level = 0.95, na.action = na.omit)$p.value
