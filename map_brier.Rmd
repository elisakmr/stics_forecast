...
for (i in 1:ngrids)  { 
    ...
    else { wbrier <- wilcox.test(score_boot[i,quant,], mu = 0, alternative = "greater", conf.level = 0.95) # h0 represented by "mu = 0" & "alternative = greater", alternative being h1
...
theme(legend.direction = "horizontal", legend.position = "right", legend.title = element_text(), legend.text=element_text(size=6),  
  ...
  labs(title = paste0(indicateur," Brier Skill Score ", extremes[quant], " ", kantile, scale))
