...
Reminder of the score array structure (first dimension details)
...
for (i in 1:ngrids)  { 
    ...
    else { wmsess <- wilcox.test(score_boot[i,quant,], mu = 0, alternative = "greater", conf.level = 0.95) # h0 represented by "mu = 0.5" & "alternative = greater", alternative being h1
...
ggmap(back_map) +
geom_sf(data = shp_grid, aes(color = bss), size=2.15, shape = 15) + 
...
theme(legend.direction = "horizontal", legend.position = "right", legend.title = element_text(), legend.text=element_text(size=6),  
  ...
  labs(title = paste0(indicateur," Brier Skill Score ", extremes[quant], " ", kantile))
