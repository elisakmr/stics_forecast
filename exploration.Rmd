...
# naming time integration - depends on time_windows (for plot titles)
scale <- " MJJASO"
...
ggplot() + geom_boxplot(df_raw %>% filter(type=="prevision"), mapping=aes(x=year, y=value), color="#dd1c77", show.legend = TRUE)+
...
p <- ggplot(df_quant, aes(x=year, y=value)) + 
  labs(title = paste0(indicateur, scale, " grid ", maille), x="Year", y= "Value")+
      theme_bw()+
