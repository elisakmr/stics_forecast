...
index_clima <- which(climat_vect==climat_select)+2
...
for (i in 1:ngrids)  { 
  ...
  else {wmsess <- wilcox.test(score_ps[i,4,]-score_ps[i,3,], mu = 0, alternative = "greater", conf.level = 0.95) # h0 represented by "mu = 0" & "alternative = greater", alternative being h1. We assess RMSE ps > RMSE clim
...
pdf(paste0(dir_plot,"/rmse_",indicateur,scalename(selected_project,temporalite,agreg),climat_select,".pdf"))
...
theme(legend.direction = "horizontal", legend.position = "right", legend.title = element_text(),  
  ...
  labs(title = paste0(indicateur,scalename(selected_project,temporalite,agreg),"RMSE_",climat_select))
print(p)
