---
title: "Probabilistic scoring regarding quintiles"
...
foreach(b=1:nboot, .packages = 'easyVerification') %dopar%  { # loop on bootstrap samples
  ...
  scoreprob_seas_ps[,1] <- veriApply("EnsRoca", fcst=seas_prev, obs=seas_obs, prob=1:4/5, parallel = TRUE)$cat1 
  scoreprob_seas_ps[,2] <- veriApply("EnsRoca", fcst=seas_prev, obs=seas_obs, prob=1:4/5, parallel = TRUE)$cat5 
   
  nom_prevseas <- paste0(indicateur, b,"scorequintseason_grid_prev.RData")
...
for (indicateur in indic_period){
  ...
  nom_seas <- paste0(indicateur,"boxquint_seas.RData")
...
indicateur <- "DRAIN"
  ...
  boxboot <- array(unlist(list_boot2) , c(ev,nboot))
   ...
   wilcox.test(boxboot[5,], mu = 0.5, alternative = "greater", conf.level = 0.95, na.action = na.omit)$p.value
