...
matrix4shp[,1] <- apply(score_ps[,1,], 1, mean, na.rm = TRUE) #  mean of bootstrapped ACC  

    # Wilcoxon test

for (i in 1:ngrids)  { 
  if (length(which(is.na(score_ps[i,1,]))) >= na_limit) {matrix4shp[i,2] <- 0; matrix4shp[i,1] <- NA}
  else { wacc <- wilcox.test(score_ps[i,1,], mu = 0.3, alternative = "greater", conf.level = 0.95) # h0 represented by "mu = 0.3" & "alternative = greater", alternative being h1
...
theme(legend.direction = "horizontal", legend.position = "right", legend.title = element_text(),  
  ...
  labs(title = paste0(indicateur,scale," acc "))
